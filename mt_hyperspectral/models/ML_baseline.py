...
class MPLSRMultiOutputModel:
    """
    多输出PLSR模型类 - 使用单个模型同时预测所有目标变量
    """
    def __init__(self, n_components=10, max_iter=500):
        self.n_components = n_components
        self.max_iter = max_iter
        self.model = None  # 单个PLSR模型用于所有目标
        self.scaler_X = StandardScaler()  # 特征缩放器
        self.scaler_y = StandardScaler()  # 目标变量缩放器
    ...
    def fit(self, X, y, target_names):
        ...
        self.model = PLSRegression(
            n_components=min(self.n_components, X.shape[1], X.shape[0], y.shape[1]),
            max_iter=self.max_iter
        )
    ...
    def predict(self, X, target_names=None):
        ...
        预测
        X: 输入特征
        target_names: 目标变量名称列表（保持接口一致性，实际不使用）
        返回: 预测结果数组，形状为 [n_samples, n_targets]
        """
        # 标准化输入特征
        X_scaled = self.scaler_X.transform(X)
    ...
    def get_feature_importance(self, target_names=None):
        ...
        # 对于多输出PLSR，每个目标变量的特征重要性基于对应的回归系数
        coef_matrix = self.model.coef_  # 形状为 [n_features, n_targets]
        
        for i, target_name in enumerate(target_names):
            # 使用第i列系数的绝对值作为特征重要性
            importance = np.abs(coef_matrix[:, i])
            feature_importance[target_name] = importance
    ...
    def get_model_info(self):
        ...
        return {
            ...
            'y_weights_shape': self.model.y_weights_.shape,
            'x_loadings_shape': self.model.x_loadings_.shape,
            'y_loadings_shape': self.model.y_loadings_.shape
        }
